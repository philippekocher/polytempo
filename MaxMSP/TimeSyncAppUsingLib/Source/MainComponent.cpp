...
{
    uint32_t t;
    bool ok = polytempo_getTime(&t) == TIME_SYNC_OK;
...
void MainComponent::buttonClicked(Button* btn)
{
    ...
    {
        polytempo_setClientName(textClientName->getText().toStdString());
    }
    else if(btn == btnSendCommand.get())
    {
        polytempo_sendEvent(textCommand->getText().toStdString());
    }
}
...
void MainComponent::processEvent(std::string const& message)
{
    ...
    uint32_t t;
    const String timeString = (polytempo_getTime(&t) == TIME_SYNC_OK) ? String(t * 0.001, 3) : "-";
